...
- **`de1` hardening TODO (Phase 3-DE, after the 22.04 rebuild):** the node currently has **password login still
  enabled** and **no active firewall** (ufw inactive). Once key access is confirmed on the 22.04 reinstall, disable
  SSH password login and apply the firewall plan. Not changed during read-only preflight (scope). See
