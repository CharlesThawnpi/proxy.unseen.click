...
- **`de1` hardening TODO (Phase 3-DE):** as of the 22.04 re-verify (2026-06-15), `de1` has **ufw ACTIVE** (INPUT DROP,
  SSH allowed) — good baseline — but **password SSH login is still enabled**. Once key access is confirmed (it is),
  disable SSH password login. When Hiddify installs, ensure its ports are allowed through ufw **without** closing SSH
  (Hiddify manages its own iptables; verify coexistence). Not changed during read-only preflight (scope). See
