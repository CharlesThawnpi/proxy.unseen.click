...
- **Firewall is Hiddify-managed (iptables).** Don't run a competing ufw/nft ruleset that fights it; verify SSH:22
  survives from a second session post-install and keep a provider recovery path (B3).
- **Install is snapshot-gated (B2)** and host-level — never run blind; prefer the Docker/remapped-port path to
  shrink blast radius. **Reality private keys/short-ids** generated by Hiddify remain secrets (rule 1).
